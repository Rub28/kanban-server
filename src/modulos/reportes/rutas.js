...
router.post('/generaFolio', generaFolio);
router.post('/clientesactivos', clientesActivos); 
router.post('/movimientos',movimientos);      
router.post('/entregaClientes', entregaClientes); 
...
async function entregaClientes (req,res, next){
   try {
      console.log("entra aca")
      const items = await controlador.entregaClientes(req.body)
...
async function clientesActivos(req,res, next){
   try {
      console.log("entra aca")
      const items = await controlador.clientesActivos(req.body)
      respuestas.success(req, res, items, 200)
   } catch (error) {
      next(error)
   }  
...
async function generaFolio (req,res, next){
   console.log("req.body", req.body)
   try {
      const items = await controlador.generaFolio(req.body)
         respuestas.success(req, res, items, 200)
   } catch (error) {
         next(error)
   }
